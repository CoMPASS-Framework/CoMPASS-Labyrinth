...
def plot_bout_metric_comparison(index_df, features):
    ...
    df_melted = index_df[index_df["Valid_bout"] == "Valid"].melt(
        id_vars="Successful_bout", value_vars=features, var_name="Feature", value_name="Value"
    )

    plt.figure(figsize=(6, 4))
    ax = sns.boxplot(
        data=df_melted, x="Feature", y="Value", hue="Successful_bout", palette=["gray", "blue"], showfliers=False
    )
    ...
    for i, feat in enumerate(features):
        ...
        unsucc_vals = df_melted[(df_melted["Successful_bout"] == "Unsuccessful") & (df_melted["Feature"] == feat)][
            "Value"
        ]
