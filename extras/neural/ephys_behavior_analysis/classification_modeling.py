...
def run_phase_session_models(df, feature_cols, feature_name_map):
    ...
    for phase in sorted(df["Phase"].unique()):
        ...
        for held_out_session in phase_data["Session"].unique():
            ...
            shap_feats_list.append(
                pd.DataFrame(X_test_scaled, columns=[feature_name_map.get(c, c) for c in X_test.columns])
            )
...
def bootstrap_ci(y_true, y_probs, curve_type="roc", n_bootstraps=1000):
    ...
    return (
        ...
        np.percentile(interpolated, 2.5, axis=0),
        np.percentile(interpolated, 97.5, axis=0),
    )
...
def plot_ci_curves_sessionwise(
    y_true_list, prob_list, curve_type="roc", model_name="", color="#1f77b4", use_bootstrap=False
):
    if use_bootstrap:
    ...
    auc_score = np.mean(
        [
            roc_auc_score(y, p) if curve_type == "roc" else average_precision_score(y, p)
...
def summarize_and_plot_all(
    ...
    pd.Series(rf_final.feature_importances_, index=mapped_names).sort_values().plot(
        kind="barh", title="Random Forest Feature Importance"
    )
    plt.tight_layout()
    plt.show()

    pd.Series(xgb_final.feature_importances_, index=mapped_names).sort_values().plot(
    ...
    plot_ci_curves_sessionwise(
        y_true_all, prob_xgb, curve_type="roc", model_name="XGBoost", color="#1f77b4", use_bootstrap=use_bootstrap
    )
    plot_ci_curves_sessionwise(
        y_true_all, prob_rf, curve_type="roc", model_name="Random Forest", color="#2ca02c", use_bootstrap=use_bootstrap
    )
    plot_ci_curves_sessionwise(
        y_true_all,
        prob_lr,
    ...
    plt.figure(figsize=(6, 6))
    plot_ci_curves_sessionwise(
        y_true_all, prob_xgb, curve_type="pr", model_name="XGBoost", color="#1f77b4", use_bootstrap=use_bootstrap
    )
    plot_ci_curves_sessionwise(
        y_true_all, prob_rf, curve_type="pr", model_name="Random Forest", color="#2ca02c", use_bootstrap=use_bootstrap
    )
    plot_ci_curves_sessionwise(
        y_true_all,
        prob_lr,
