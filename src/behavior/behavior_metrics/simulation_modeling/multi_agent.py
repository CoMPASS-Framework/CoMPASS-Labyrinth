    ...
    for i in range(len(chunk) - 1):
        if chunk.iloc[i]['NodeType'] == decision_label:
            ...
            sim_choices = [
                ...
                for _ in range(n_simulations) if curr in valid_dict
            ]
            if sim_choices:
...
def simulate_binary_agent_multi(chunk, valid_dict, optimal_dict, decision_label, n_simulations):
    ...
    for i in range(len(chunk) - 1):
        if chunk.iloc[i]['NodeType'] == decision_label:
            ...
            binary_opt = [
                1 if random.choice(sim_choices) in opt else 0
...
def simulate_multiway_agent_multi(chunk, valid_dict, optimal_dict, decision_label, three_nodes, four_nodes, n_simulations):
    perf = []
...
def evaluate_epoch_multi(chunk, valid_dict, optimal_dict, decision_label, reward_label,
                   three_nodes, four_nodes, n_bootstrap, n_simulations):
    ...
    multiway_perf = simulate_multiway_agent_multi(chunk, valid_dict, optimal_dict, decision_label,
                                            three_nodes, four_nodes, n_simulations)
    ...
    return pd.Series({
        ...
        'Relative Performance (Actual/Random)': actual_boot.mean() / random_boot.mean() if random_boot.mean() > 0 else np.nan,
        'Relative Performance (Actual/Binary)': actual_boot.mean() / binary_boot.mean() if binary_boot.mean() > 0 else np.nan
    })


# -------------------- Step 4: Main Evaluation Wrapper -------------------- #
def evaluate_agent_performance_multi(df, epoch_size, n_bootstrap, n_simulations,
                                decision_label='Decision (Reward)', reward_label='Reward Path',
                                three_nodes=None, four_nodes=None):
    ...
    for session, idx, chunk in epochs:
        ...
        metrics = evaluate_epoch_multi(chunk, valid_dict, optimal_dict,
                                 decision_label, reward_label,
                                 three_nodes, four_nodes,
...
def plot_agent_vs_mouse_performance_multi(df_metrics, mouseinfo, genotype, figsize=(12, 6)):
    ...
    sns.lineplot(data=df_filtered, x=x_col, y=y_col_multi, linestyle="dashed", label="Three/Four Way Agent", color='maroon')
...
def plot_cumulative_agent_comparison_boxplot_multi(df_metrics, mouseinfo, genotype,figsize=(10, 6)):
    ...
    plt.title(f'Cumulative Reward Path Transition % across Sessions\nGenotype: {genotype}', fontsize=14, fontweight='bold')
