...
def compute_state_probability(df_hmm,
                               column_of_interest,
    ...
    st_cnt = df_plot.groupby(["Genotype", column_of_interest, "Session", "HMM_State"]).size().rename('cnt').reset_index()
    ...
    if values_displayed:
        ...
        state_count[column_of_interest] = pd.Categorical(state_count[column_of_interest],
                                                         categories=values_displayed, ordered=True)

    return state_count


def plot_state_probability_boxplot(state_count,
                                    column_of_interest,
                                    state=1,
                                    figsize=(14, 7),
                                    palette='Set2'):
...
def run_pairwise_ttests(state_count_df, column_of_interest='NodeType'):
    """
