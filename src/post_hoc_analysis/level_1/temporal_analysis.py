...
def compute_node_state_medians_over_time(
    ...
    for genotype in df_hmm['Genotype'].unique():
        ...
        for node_type_list, node_type_label in zip(
            [decision_nodes, nondecision_nodes],
            ['Decision node', 'Non-Decision node']
        ):
            ...
            for k in range(lower_lim, upper_lim, bin_size):
                for session_df in sess_li:
                    ...
                    subset = state_count[
                        (state_count['HMM_State'].isin(state_types)) &
                        (state_count['Grid.Number'].isin(node_type_list))
                    ]
...
def plot_node_state_median_curve(Deci_DF, palette, figure_ylimit, fig_title=None):
    """
