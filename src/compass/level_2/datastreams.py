...
def loso_kde_cv(df, smoothing_factors):
...
def plot_kde_per_session(df, best_sigma, kde_col='KDE'):
    for session in df['Session'].unique():
        ...
        plt.imshow(scaled, extent=[x_vals.min(), x_vals.max(), y_vals.min(), y_vals.max()],
...
def compute_center_coordinates(df, grid_nodes):
    """
...
def compute_value_distance(df, value_map_path, center_grids=[84, 85]):
    """
