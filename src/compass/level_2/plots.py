...
def plot_kde_per_session(df, best_sigma, kde_col='KDE'):
    for session in df['Session'].unique():
        ...
        plt.imshow(scaled, extent=[x_min, x_max, y_min, y_max],
...
def compute_spatial_embedding(df: pd.DataFrame, sigma: float = 2) -> pd.DataFrame:
    ...
    df['spatial_embedding'] = df.apply(
        lambda row: smoothed_grid[int(row['x']), int(row['y'])], axis=1
    )
...
def compute_detailed_bout_summary(df, feature_cols, node_filter='Decision (Reward)',
                                  state_col='HMM State', target_zone='Target Zone',
                                  valid_bout_threshold=10, bout_col='Bout_ID'):
    """
...
def plot_measures_by_bout_type(index_df, feature_cols=None):
    ...
    sns.violinplot(data=df_melted, x='variable', y='value', hue='Successful_bout',
                   palette=palette, split=True, inner=None, linewidth=1.2, alpha=0.8)

    sns.boxplot(data=df_melted, x='variable', y='value', hue='Successful_bout',
                palette=['blue', 'black'], width=0.3, showcaps=True,
                boxprops={'zorder': 2, 'facecolor': 'none'},
                whiskerprops={'zorder': 2}, medianprops={'zorder': 3})

    sns.stripplot(data=df_melted, x='variable', y='value', hue='Successful_bout',
                  palette=['blue', 'black'], dodge=True, alpha=0.7, jitter=True, size=4, marker='o')
