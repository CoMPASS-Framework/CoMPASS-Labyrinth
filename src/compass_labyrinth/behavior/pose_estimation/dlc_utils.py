...
def get_labyrinth_boundary_and_cropping(frames_directory, cropping_directory, 
                                       boundaries_directory, session, chamber_info=None):
    """
    ...
    def click_event(event, x, y, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if len(posList) < 4:  # Only accept 4 points
                ...
                cv2.putText(img_display, f"{corner_index + 1}: {corner_name}", 
                           (x+15, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                ...
                if len(posList) == 4:
                    ...
                    cv2.putText(img_display, f"Crop: {X2-X1}x{Y2-Y1}", 
        ...
        # Right mouse click to show pixel values
            ...
            g = img[y, x, 1]
            r = img[y, x, 2]
            cv2.putText(img_display, f"RGB: {r},{g},{b}",
                       (x+10, y+10), font, 0.4, (255, 255, 0), 1)
            cv2.imshow('Labyrinth Boundary Selection', img_display)
...
def batch_get_boundary_and_cropping(mouseinfo_df, frames_directory, cropping_directory, 
...
def analyze_videos_with_DLC(mouseinfo_df, config_path, video_directory, 
                           cropping_directory, results_directory):
    """
    ...
    for index, row in mouseinfo_df.iterrows():
        ...
        try:
            ...
            analysis_summary['analysis_times'].append({
                'session': session_name,
...
def get_grid_coordinates(posList, num_squares, grid_files_directory, session, cropping_coords=None):
    ...
    if cropping_coords is not None:
        ...
        print(f"  Original boundary bounds: X({border[:, 0].min():.0f}-{border[:, 0].max():.0f}), Y({border[:, 1].min():.0f}-{border[:, 1].max():.0f})")
        print(f"  Adjusted boundary bounds: X({adjusted_border[:, 0].min():.0f}-{adjusted_border[:, 0].max():.0f}), Y({adjusted_border[:, 1].min():.0f}-{adjusted_border[:, 1].max():.0f})")
...
def batch_create_grids(mouseinfo_df, boundaries_directory, grid_files_directory, 
                      cropping_directory, num_squares=12):
    """
    ...
    for index, row in mouseinfo_df.iterrows():
        ...
        try:
            ...
            cropping_coords = (cropping_data['X1'], cropping_data['X2'], 
                             cropping_data['Y1'], cropping_data['Y2'])
...
def create_grid_scatter_plot(session, dlc_results_directory, grid_files_directory, 
                            dlc_scorer, bodypart='sternum', likelihood_threshold=0.6,
                            figure_size=(3, 3), save_plot=True, figures_directory=None):
    """
    ...
    ax.text(0.02, 0.98, f"Points: {valid_points}/{total_points} ({valid_points/total_points*100:.1f}%)",
...
def batch_create_grid_scatter_plots(mouseinfo_df, dlc_results_directory, grid_files_directory, 
                                   figures_directory, dlc_scorer, bodypart='sternum', 
                                   likelihood_threshold=0.6, figure_size=(3, 3), 
                                   show_plots=False):
    """
...
def create_trajectory_plot(session, dlc_results_directory, grid_files_directory, 
                          dlc_scorer, bodypart='sternum', likelihood_threshold=0.6,
                          figure_size=(4, 4), colormap='viridis', save_plot=True, 
...
def batch_create_trajectory_plots(mouseinfo_df, dlc_results_directory, grid_files_directory, 
                                 figures_directory, dlc_scorer, bodypart='sternum', 
                                 likelihood_threshold=0.6, figure_size=(4, 4), 
                                 colormap='viridis', show_plots=False):
    """
...
def append_grid_numbers_to_csv(session, dlc_results_directory, grid_files_directory, 
                              dlc_scorer, bodyparts=['nose', 'belly', 'sternum', 'leftflank', 'rightflank', 'tailbase'],
                              save_directory=None):
    """
...
def batch_append_grid_numbers(mouseinfo_df, dlc_results_directory, grid_files_directory, 
                             dlc_scorer, bodyparts=['nose', 'belly', 'sternum', 'leftflank', 'rightflank', 'tailbase'],
                             save_directory=None):
    """
