...
def compute_epoch_metrics(df_slice, valid_dict, optimal_dict, n_bootstrap, n_simulations, decision_label='Decision (Reward)'):
    ...
    if df_slice.empty or decision_label not in df_slice['NodeType'].values:
        return pd.Series({k: np.nan for k in [
            'Actual Reward Path %', 'Simulated Agent Reward Path %',
            'Actual Reward Path % CI Lower', 'Actual Reward Path % CI Upper',
            'Simulated Agent Reward Path % CI Lower', 'Simulated Agent Reward Path % CI Upper',
    ...
    return pd.Series({
        ...
        'Relative Performance': np.mean(actual_dist) / np.mean(simulated_dist) if np.mean(simulated_dist) > 0 else np.nan
    })
...
def evaluate_agent_performance(df, epoch_size, n_bootstrap, n_simulations, decision_label='Decision (Reward)', reward_label='Reward Path'):
...
def plot_agent_transition_performance(df_result):
    ...
    sns.lineplot(data=df_result, x="Epoch Number", y="Simulated Agent Reward Path %", linestyle="dashed", label="Simulated Agent", color='navy')
...
def plot_agent_transition_performance_by_genotype(df_all_csv: pd.DataFrame,
                                                  epoch_size=1000,
                                                  n_bootstrap=10000,
                                                  n_simulations=100,
    ...
    for i, genotype in enumerate(genotypes):
        ...
        sns.lineplot(data=df_result, x="Epoch Number", y="Actual Reward Path %",
                     marker="o", label="Mouse", color='black', ax=ax)
        sns.lineplot(data=df_result, x="Epoch Number", y="Simulated Agent Reward Path %",
                     linestyle="dashed", label="Simulated Agent", color='navy', ax=ax)
...
def plot_relative_agent_performance_by_genotype(df_all_csv: pd.DataFrame,
                                                epoch_size=1000,
                                                n_bootstrap=10000,
                                                n_simulations=100,
                                                decision_label="Decision (Reward)",
                                                reward_label="Reward Path",palette=None):
    genotypes = df_all_csv['Genotype'].unique()
    n_genotypes = len(genotypes)
    ...
    for i, genotype in enumerate(genotypes):
        ...
        if palette is None:
            palette='black'
        sns.lineplot(data=df_result, x="Epoch Number", y="Relative Performance",
                     marker="o", color=palette[i], ax=ax)
...
# -----------------------------------------
def run_mixedlm_for_all_genotypes(df_all_csv: pd.DataFrame,
                                   epoch_size=1000,
                                   n_bootstrap=10000,
                                   n_simulations=100,
                                   decision_label="Decision (Reward)",
...
def compute_chi_square_statistic(df):
    ...
    """
    df = df.copy()
    chi_square = ((df['Actual Reward Path %'] - df['Simulated Agent Reward Path %']) ** 2) / df['Simulated Agent Reward Path %']
...
def compute_rolling_chi_square(df, window=3):
    ...
    df = df.copy()
    df['Rolling Chi Square'] = df.groupby('Session')['Chi Square Statistic'].transform(lambda x: x.rolling(window=window, min_periods=1).mean())
    return df
...
def plot_chi_square_and_rolling(df, epoch_col='Epoch Number', chi_col='Chi Square Statistic',
                                 rolling_col='Rolling Chi Square', session_col='Session'):
    ...
    sns.lineplot(data=df, x=epoch_col, y=rolling_col, hue=session_col,
                 palette='coolwarm', alpha=0.7, lw=2, legend='full')
    plt.xlabel('Epochs')
...
def run_chi_square_analysis_by_genotype(df_all_csv,
                                        epoch_size=1000,
                                        n_bootstrap=10000,
                                        n_simulations=100,
                                        decision_label="Decision (Reward)",
...
def plot_chi_square_and_rolling_subplots(df, epoch_col='Epoch Number', chi_col='Chi Square Statistic',
