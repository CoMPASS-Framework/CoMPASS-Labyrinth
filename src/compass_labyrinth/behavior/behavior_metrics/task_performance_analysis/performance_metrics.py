...
def plot_target_usage_vs_frames(
    ...
    for line in range(summary_df.shape[0]):
        plt.text(
            ...
            ha='right', color='black', size='medium'
        )
...
def plot_region_heatmaps(
    ...
    fig.suptitle(
        f"Region Occupancy Heatmaps for {group_name}",
        fontsize=18,
        weight='bold',
        y=0.99
    )
...
def run_mixed_model_per_genotype_pair(entropy_df):
    ...
    for g1, g2 in genotype_pairs:
        ...
        try:
            ...
            interaction_pvals = {
                k: v for k, v in result.pvalues.items() if "Bin" in k and "Genotype" in k
            }
...
def plot_region_usage_over_bins(region_data, region_name, palette=None, ylim=(0, 1)):
    ...
    """
    ax = sns.catplot(
        x='Bin', y=region_name, hue='Genotype',
        data=region_data, kind="point", capsize=.15,
        errorbar='se', palette=palette, aspect=1.5
    )
