...
def prep_data(
    data: pd.DataFrame,
    type: str = "UTM",                      # "UTM" or "LL"
    coordNames = ("x", "y"),
    covNames = None,
    centers: np.ndarray | None = None,     # shape (K, 2)
    centroids: dict | None = None,         # dict[name] -> DataFrame with ["x","y", time_col]
    angleCovs = None,                       # included with covariates (no circular transform here)
    ...
    if centroids is not None:
        ...
        for name, cdf in centroids.items():
            ...
            for _id, g in df.groupby("ID", sort=False):
                idx = g.index
                x = g[xcol].to_numpy(); y = g[ycol].to_numpy()
                cx = g[f"__{name}_x"].to_numpy(); cy = g[f"__{name}_y"].to_numpy()
                dist_vals = np.full(len(g), np.nan)
                ang_vals  = np.full(len(g), np.nan)
                for k in range(1, len(g)):
